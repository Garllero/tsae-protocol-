...
//	private transient LSimWorker lsim = LSimFactory.getWorkerInstance();
	...
	public synchronized void updateTimestamp(Timestamp timestamp){
		...
		/*update the vector with the new timestamp*/
	...
	public synchronized void updateMax(TimestampVector tsVector){
		...
		/*We start looking in the vector*/
		 for (String node : tsVector.timestampVector.keySet()) {
			 Timestamp inputTimestamp = tsVector.timestampVector.get(node);
			 Timestamp currentTimestamp = timestampVector.get(node);
			 /*Compare the current node we are looking with the node from the vector*/
		 	if (currentTimestamp == null || currentTimestamp.compare(inputTimestamp) < 0) {
		 		/*if currentTimestamp is older than inputTimestamp, we actualized it (because it means that input is newer)*/
		 		timestampVector.put(node, inputTimestamp);
		 	}
		 }	
	}
	...
	public Timestamp getLast(String node){
		/*Check the nodes to get the last one*/
		if(this.timestampVector.containsKey(node)) {
	 ...
	 */
	public synchronized void mergeMin(TimestampVector tsVector){  //FASE 3//
		//tsVector is not null
		if(tsVector == null) {
			return;
			...
			for (Map.Entry<String, Timestamp> entry:tsVector.timestampVector.entrySet()) {
				//Create variables and save the actual key and his timestamps
				String key = entry.getKey();
				Timestamp firstTs = this.timestampVector.get(key);
				Timestamp lastTs = tsVector.getLast(key);
				//if the first 
				if(firstTs.compare(lastTs)>0) {
	...
	TimestampVector cloningtime = new TimestampVector(Arrays.asList(participants));
	
	for (String node:this.timestampVector.keySet()) {
		Timestamp theTimestamp = this.getLast(node);
		cloningtime.updateTimestamp(theTimestamp);
	}
	return cloningtime;
	}
	...
	public boolean equals(Object obj){
		...
		if(obj !=null && obj instanceof TimestampVector) {
			...
			TimestampVector timeVector = (TimestampVector) obj;
			return timestampVector.equals(timeVector.timestampVector);
		}
