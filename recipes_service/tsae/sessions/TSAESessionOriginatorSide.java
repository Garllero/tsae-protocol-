...
public class TSAESessionOriginatorSide extends TimerTask{
	...
	private void sessionTSAE(Host n){
		...
		try {
			...
			List<Operation> opPartner = serverData.getLog().listNewer(msgAReq.getSummary());
				// send operations
			for(Operation ops : opPartner) {
				...
				if (msg.type() == MsgType.END_TSAE){
					synchronized (serverData) {
						for (MessageOperation ops : OperationList) {//fase 4//
								serverData.execOpRemoveAdd(ops.getOperation());
						} 
						//Update summary and Ack //fase2//
