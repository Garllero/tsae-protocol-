	...
	public void run() {
		...
		try {
			...
			if (msg.type() == MsgType.AE_REQUEST){
				MessageAErequest msgAReq = (MessageAErequest)msg;
				TimestampVector localSummary;
				TimestampMatrix localAck;
				/*Compilating locakSummary*/
				
				/*synchronizing it with the data from the server */
				...
				// send operations
				List<Operation> logs = serverData.getLog().listNewer(msgAReq.getSummary());
				for(Operation ops: logs) {
					out.writeObject(new MessageOperation(ops));
	            ...
	            // receive operations
				List <MessageOperation> OperationList = new Vector<MessageOperation>();
				msg = (Message) in.readObject();
				LSimLogger.log(Level.TRACE, "[TSAESessionPartnerSide] [session: "+current_session_number+"] received message: "+ msg);
				...
				synchronized (serverData) {
					//fase4//
					for (MessageOperation ops : OperationList) {
							serverData.execOpRemoveAdd(ops.getOperation());
						
					} 
					//Update Summary and ack //fase 2//
					serverData.getSummary().updateMax(msgAReq.getSummary());
					//fase3//
					serverData.getAck().updateMax(msgAReq.getAck());
