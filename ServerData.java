...
//		LSimLogger.log(Level.TRACE,"The recipe '"+recipeTitle+"' has been added");
	...
	public synchronized void removeRecipe(String recipeTitle){ //fase4//
	//Save the title and timestamp of the parametric recipe
		Timestamp timestamp = nextTimestamp();
		Recipe rcp = this.recipes.get(recipeTitle);
		
		//New op, remove type with the variables calculated before
		Operation remOp = new RemoveOperation(recipeTitle, rcp.getTimestamp(),timestamp);
	...
	public synchronized void execOpRemoveAdd (Operation removeOrAddOperation) { //fase4//
